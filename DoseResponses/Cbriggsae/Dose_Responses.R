...
setwd("~/Dropbox/Andersenlab/LabFolders/Mostafa/Manuscripts/Benzimidazoles/Github-BZ/DoseResponses/Cbriggsae")
load("Cb_BZ_DR.Rda")
source("~/Dropbox/Andersenlab/LabFolders/Mostafa/Manuscripts/Benzimidazoles/Github-BZ/DoseResponses/Cbriggsae/Dose_Response_Fxns.R")
all_herits <- list()
...
#albendazole: 25, fenbendazole: 30, thiabendazole: 40
  
plotdf <- dr_H2 %>%
  mutate(h2_labl = ifelse(condition == "Albendazole" & dose_µM == 25 & strain == "AF16", "plot",
                          ifelse(condition == "Fenbendazole" & dose_µM == 30  & strain == "AF16", "plot",
                                        ifelse(condition == "Thiabendazole" & dose_µM == 31.25 | dose_µM == 62.5 & strain == "AF16", "plot",NA))))
...
plot <- ggplot(plotdf, aes(dose_µM,mean_phenotype, colour = strain)) +
  ...
  scale_color_manual(values = c("AF16" = "indianred", "HK104"= "gold", "ED3035" = "green4", "VT847" = "purple"))+
  ...
  geom_text(data=subset(plotdf,h2_labl =="plot" & trait == "Length"),aes(y=300, label = as.character(signif(H2,2))), color= "black", size = 3.5) +
  geom_text(data=subset(plotdf,h2_labl =="plot" & trait == "Brood Size"),aes(y=15, label = as.character(signif(H2,2))), color= "black", size = 3.5) +
  geom_text(data=subset(plotdf,h2_labl =="plot" & trait == "Pharyngeal Pumping"),aes(y=75, label = as.character(signif(H2,2))), color= "black", size = 3.5) +
...
ggsave(plot = plot, filename = "Cb_DR.tiff", width = 12, height = 8)
